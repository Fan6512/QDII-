...
const DETAIL_FIELDS = [
  "management_fee", "custody_fee", "fund_scale_billion", "fund_scale_date",
  "inception_date", "performance_as_of", "return_3m", "return_6m",
  "return_1y", "return_5y", "since_inception_annualized",
  "tracking_error_annualized", "tracking_error_as_of",
];

export function mergeFundDetails(current, incoming) {
  const merged = { ...current };
  for (const field of DETAIL_FIELDS) {
    if (incoming[field] !== null && incoming[field] !== undefined) merged[field] = incoming[field];
  }
  for (const field of ["limit_daily", "fee_original", "fee_discount", "min_subscribe"]) {
    if (incoming[field] !== null && incoming[field] !== undefined) merged[field] = incoming[field];
  }
  if (incoming.status && incoming.status !== "unknown") merged.status = incoming.status;
  return merged;
}

export function normalizeFund(candidate) {
  return mergeFundDetails({
    code: candidate.code,
